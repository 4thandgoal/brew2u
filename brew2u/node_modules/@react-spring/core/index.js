...
import { is, usePrev, useOnce, useForceUpdate, Globals } from '@react-spring/shared';
...
import { AnimatedValueArray, AnimatedInterpolation, AnimatedValue, Animated } from '@react-spring/animated';
  ...
  _animate(props) {
    ...
    for (const key in this.merged) {
      ...
      if (props.reset || isAttaching || !isEqual(goalValue, state.isNew ? currValue : state.goalValue)) {
        ...
        const toValues = parent && getPayload(parent, key) || toArray(isInterpolated && 1 || goalValue);
        changed = true;
    ...
    if (changed) {
      if (started.length) {
        ...
        if (onStart) started.forEach(key => {
  ...
  constructor(_temp) {
    ...
    this.update = update && update.bind(this) || (() => {
      ...
      for (const id of Array.from(this.controllers.keys())) {
        ...
        const changes = ctrl.props.onFrame && [];
