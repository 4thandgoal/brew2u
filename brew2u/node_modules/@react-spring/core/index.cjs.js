...
var animated = require('@react-spring/animated');
...
function toArray(a) {
  return shared.is.und(a) ? [] : Array.isArray(a) ? a : [a];
}
...
function getForwardProps(props) {
  var children = props.children,
      ...
      attach = props.attach,
      forward = _objectWithoutPropertiesLoose(props, ["children", "config", "from", "to", "ref", "reset", "cancel", "reverse", "immediate", "delay", "lazy", "items", "trail", "unique", "initial", "enter", "leave", "update", "onStart", "onRest", "onFrame", "onDestroyed", "timestamp", "attach"]);
...
function () {
  ...
  _proto.onFrame = function onFrame(_ref3) {
    var id = _ref3[0],
        isActive = _ref3[1],
        entries = _ref3[2];
  ...
  _proto._attach = function _attach(keys, visited) {
    ...
    this.children.forEach(function (c) {
      ...
      var attached = keys.filter(function (key) {
        var payload = getPayload(c, key);

        if (payload) {
          payload.forEach(function (v) {
            return v.done && v.reset(true);
          });
          return true;
        }
      });
  ...
  _proto._runAsync = function _runAsync(_ref6, onEnd) {
    var _this4 = this;

    var to = _ref6.to,
        props = _objectWithoutPropertiesLoose(_ref6, ["to"]);
  ...
  _proto._diff = function _diff(_ref7) {
    var _this5 = this;
    ...
    var timestamp = _ref7.timestamp,
        ...
        attach = _ref7.attach,
        props = _objectWithoutPropertiesLoose(_ref7, ["timestamp", "delay", "config", "immediate", "reverse", "attach"]);
  ...
  _proto._animate = function _animate(props) {
    ...
    for (var key in this.merged) {
      ...
      if (props.reset || isAttaching || !isEqual(goalValue, state.isNew ? currValue : state.goalValue)) {
        var _ret2 = function () {
          ...
          if (isInterpolated) {
            var output = [props.reset ? fromValue : animated$1.getValue(), goalValue];
            ...
            if (shared.is.arr(value)) {
              if (animated$1 instanceof animated.AnimatedValueArray) {
          ...
          var toValues = parent && getPayload(parent, key) || toArray(isInterpolated && 1 || goalValue);
          changed = true;
          animatedValues = toArray(animated$1.getPayload());
    ...
    if (changed) {
      if (started.length) {
        ...
        if (onStart) started.forEach(function (key) {
...
function createAnimated(value) {
  return shared.is.arr(value) ? new animated.AnimatedValueArray(value.map(function (value) {
    var animated$1 = createAnimated(value);
    var payload = animated$1.getPayload();
    return animated$1 instanceof animated.AnimatedInterpolation ? payload[0] : payload;
  })) : isAnimatableString(value) ? // Convert "red" into "rgba(255, 0, 0, 1)" etc
  new animated.AnimatedValue(0).interpolate({
    output: [value, value]
...
function getPayload(ctrl, key) {
  var anim = ctrl.animations[key];
...
var makeConfig = function makeConfig(props) {
  ...
  items = toArray(shared.is.und(items) ? null : items);
  return _extends({
    items: items,
...
function () {
  function FrameLoop(_temp) {
    ...
    this.update = update && update.bind(this) || function () {
      ...
      for (var _i = 0, _Array$from = Array.from(_this.controllers.keys()); _i < _Array$from.length; _i++) {
        ...
        var changes = ctrl.props.onFrame && [];
  ...
  _proto.advance = function advance(config) {
    var time = G.now();
    var isActive = false;
    var finished = false;
